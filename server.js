...
io.on('connection', (socket) => {
  ...
  // توزيع الشخصيات بدون اشتراط لاعب مستقل لكل شخصية.
  // إذا كان عندنا شخصيتان واثنان من المؤدين فكل واحد يأخذ شخصية،
  // وإذا كان عندنا 3 شخصيات ومؤديان فالشخصية الثالثة تُسند تلقائياً لأحدهما.
  // المهم: لا يبقى أي دور "معلقاً" بانتظار لاعب ثالث.
  function autoAssignCharacters(room) {
    if (!room.players.length || !room.characters || room.characters.length === 0) {
      room.players.forEach(p => { p.assignedCharacters = []; p.assignedCharacter = null; });
      return;
    }

    room.players.forEach(player => {
      player.assignedCharacters = [];
      player.assignedCharacter = null;
    });

    room.characters.forEach((character, index) => {
      const player = room.players[index % room.players.length];
      player.assignedCharacters.push(character);
    });

    room.players.forEach(player => {
      player.assignedCharacter = player.assignedCharacters[0] || null;
    });

    room.turns.forEach(turn => {
      const assignedPlayer = room.players.find(p => (p.assignedCharacters || []).includes(turn.characterName));
      if (assignedPlayer) {
  ...
  socket.on('stop_recording_sync', ({ audioBase64, duration }) => {
    if (!currentRoom || !rooms[currentRoom]) return;
    const room = rooms[currentRoom];
