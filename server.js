...
io.on('connection', (socket) => {
  ...
  // مزامنة بدء التسجيل الجماعي (المقطع يشتغل للكل)
  ...
  // مزامنة الاستماع للكل (المقطع يشتغل للجميع مكتوم الأصوات الأصلية مع تشغيل صوت الدوبلاج)
  ...
  // الانتقال للدور التالي جماعياً للجميع
  socket.on('next_turn', () => {
    if (!currentRoom || !rooms[currentRoom]) return;
    const room = rooms[currentRoom];
  ...
  // تشغيل الفيديو النهائي بالدوبلاج للجميع متزامن
...
server.listen(PORT, () => {
  console.log(`🚀 Choicer Synchronized Dubbing Server running at http://localhost:${PORT}`);
});
